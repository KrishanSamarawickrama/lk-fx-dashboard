...
public class HsbcPdfScraperTests
{
    private static byte[] LoadPdfFixture()
    {
        var assembly = typeof(HsbcPdfScraperTests).Assembly;
        using var stream = assembly.GetManifestResourceStream(
            "LkFxDashboard.Tests.Fixtures.hsbc-rates.pdf")!;
        using var ms = new MemoryStream();
        stream.CopyTo(ms);
        return ms.ToArray();
    }

    private static HsbcPdfScraper CreateScraper(byte[] pdfBytes)
    {
        var httpClient = MockHttpMessageHandlerForBytes.CreateClient(pdfBytes);
        var options = Options.Create(new ScraperOptions());
        var logger = NullLogger<HsbcPdfScraper>.Instance;
        return new HsbcPdfScraper(httpClient, options, logger);
    }
    ...
    public void ParsePdf_CorrectSource()
    {
        ...
        rates.Should().AllSatisfy(r =>
            r.Source.Should().Be(ScraperSource.Hsbc));
    }
    ...
    [Fact]
    public async Task FetchRatesAsync_OnHttpError_ReturnsEmpty()
    {
        var httpClient = MockHttpMessageHandlerForBytes.CreateClient(
            [], System.Net.HttpStatusCode.NotFound);
        var options = Options.Create(new ScraperOptions());
        var logger = NullLogger<HsbcPdfScraper>.Instance;
        var scraper = new HsbcPdfScraper(httpClient, options, logger);
    ...
    public void SourceName_IsCorrect()
    {
        ...
        scraper.SourceName.Should().Be(ScraperSource.Hsbc);
    }
}
